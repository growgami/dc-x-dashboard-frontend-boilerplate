...
const Grid3Card = forwardRef<HTMLDivElement, Grid3CardProps>(function Grid3Card({ onClick }, ref) {
  ...
  if (error) {
    return (
      <Card
        ref={ref}
        className="col-start-3 row-start-1 row-span-2 flex flex-col h-full transition-all duration-300 hover:shadow-xl shadow-[-2px_-2px_8px_#ffffff,8px_8px_16px_#d1d1d1] cursor-pointer"
        onClick={onClick}
      >
