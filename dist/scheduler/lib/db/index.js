"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.getDb = getDb;
exports.pool = getPool;
var pg_1 = require("pg");
function getPool() {
    if (!global._pgPool ||
        (typeof global._pgPool === 'object' &&
            ((Object.prototype.hasOwnProperty.call(global._pgPool, '_ending') && global._pgPool._ending) ||
                (Object.prototype.hasOwnProperty.call(global._pgPool, '_closed') && global._pgPool._closed)))) {
        global._pgPool = new pg_1.Pool({
            connectionString: process.env.POSTGRES_URL,
        });
    }
    return global._pgPool;
}
var initialized = false;
function initializeSchema() {
    return __awaiter(this, void 0, void 0, function () {
        var err_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    if (initialized)
                        return [2 /*return*/];
                    console.log('[DB] Initializing schema...');
                    _a.label = 1;
                case 1:
                    _a.trys.push([1, 3, , 4]);
                    // Create tables if they don't exist (PostgreSQL syntax)
                    return [4 /*yield*/, getPool().query("\n      CREATE TABLE IF NOT EXISTS tweets (\n        id SERIAL PRIMARY KEY,\n        tweet_id TEXT UNIQUE NOT NULL,\n        url TEXT NOT NULL,\n        text TEXT NOT NULL,\n        post_date TEXT NOT NULL,\n        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,\n        updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP\n      );\n\n      CREATE TABLE IF NOT EXISTS tweet_metrics (\n        id SERIAL PRIMARY KEY,\n        tweet_id TEXT NOT NULL,\n        likes INTEGER NOT NULL,\n        retweets INTEGER NOT NULL,\n        replies INTEGER NOT NULL,\n        views INTEGER NOT NULL,\n        quotes INTEGER NOT NULL,\n        bookmarks INTEGER NOT NULL,\n        collected_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,\n        FOREIGN KEY (tweet_id) REFERENCES tweets(tweet_id)\n      );\n\n      CREATE TABLE IF NOT EXISTS engagement_metrics (\n        id SERIAL PRIMARY KEY,\n        date TEXT NOT NULL UNIQUE,\n        impressions INTEGER NOT NULL,\n        engagements INTEGER NOT NULL,\n        followers INTEGER NOT NULL DEFAULT 0,\n        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP\n      );\n\n      CREATE INDEX IF NOT EXISTS idx_tweets_tweet_id ON tweets(tweet_id);\n      CREATE INDEX IF NOT EXISTS idx_tweet_metrics_tweet_id ON tweet_metrics(tweet_id);\n      CREATE INDEX IF NOT EXISTS idx_engagement_metrics_date ON engagement_metrics(date);\n    ")];
                case 2:
                    // Create tables if they don't exist (PostgreSQL syntax)
                    _a.sent();
                    console.log('[DB] Schema initialized successfully.');
                    initialized = true;
                    return [3 /*break*/, 4];
                case 3:
                    err_1 = _a.sent();
                    console.error('[DB] Schema initialization failed:', err_1);
                    throw err_1;
                case 4: return [2 /*return*/];
            }
        });
    });
}
// Only end the pool in production on shutdown
if (process.env.NODE_ENV === 'production') {
    var signalHandlersRegistered = false;
    if (!signalHandlersRegistered) {
        process.on('SIGTERM', function () { return __awaiter(void 0, void 0, void 0, function () {
            var err_2;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 3, , 4]);
                        if (!global._pgPool) return [3 /*break*/, 2];
                        return [4 /*yield*/, global._pgPool.end()];
                    case 1:
                        _a.sent();
                        _a.label = 2;
                    case 2:
                        console.log('[DB] Pool ended gracefully on SIGTERM');
                        return [3 /*break*/, 4];
                    case 3:
                        err_2 = _a.sent();
                        console.error('[DB] Error ending pool on SIGTERM:', err_2);
                        return [3 /*break*/, 4];
                    case 4: return [2 /*return*/];
                }
            });
        }); });
        process.on('SIGINT', function () { return __awaiter(void 0, void 0, void 0, function () {
            var err_3;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 3, , 4]);
                        if (!global._pgPool) return [3 /*break*/, 2];
                        return [4 /*yield*/, global._pgPool.end()];
                    case 1:
                        _a.sent();
                        _a.label = 2;
                    case 2:
                        console.log('[DB] Pool ended gracefully on SIGINT');
                        return [3 /*break*/, 4];
                    case 3:
                        err_3 = _a.sent();
                        console.error('[DB] Error ending pool on SIGINT:', err_3);
                        return [3 /*break*/, 4];
                    case 4: return [2 /*return*/];
                }
            });
        }); });
        signalHandlersRegistered = true;
    }
}
function getDb() {
    return __awaiter(this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, initializeSchema()];
                case 1:
                    _a.sent();
                    return [2 /*return*/, getPool()];
            }
        });
    });
}
